...
async fn compress_creates_tar_gz() -> Result<()> {
    ...
    let result = compress_to_tar_gz_large(&file_path, None).await?;
...
async fn decompress_restores_file() -> Result<()> {
    ...
    let compress_result = compress_to_tar_gz_large(&file_path, None).await?;
...
async fn decompress_multiple_files() -> Result<()> {
    ...
    let compress1 = compress_to_tar_gz_large(&file1, None).await?;
    let compress2 = compress_to_tar_gz_large(&file2, None).await?;
