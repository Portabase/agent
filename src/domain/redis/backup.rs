...
pub async fn run(
    ...
    tokio::task::spawn_blocking(move || -> Result<PathBuf> {
        ...
        logger.log("debug", format!("Starting Redis backup for database {}", cfg.name));
        ...
        if !output.status.success() {
            if stderr.contains("NOAUTH") {
                error!("Redis backup failed for {}: Authentication required (NOAUTH)", cfg.name);
                logger.log_command(cmd_label, Some("Authentication required (NOAUTH)".into()), Some(exit_code), Some(duration_ms));
                anyhow::bail!("Redis backup failed for {}: Authentication required", cfg.name);
            } else {
        ...
        info!("Redis backup completed for {}. Output: {}", cfg.name, stdout);
