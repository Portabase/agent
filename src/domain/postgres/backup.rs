    ...
    tokio::task::spawn_blocking(move || -> Result<PathBuf> {
        ...
        match format {
            PostgresDumpFormat::Fc => {
                ...
                let cmd_label = format!("pg_dump -Fc {}", url);
                ...
                match output {
                    Ok(o) => {
                        ...
                        if o.status.success() {
                            info!("FC backup completed successfully for {} at {:?}", cfg.name, file_path);
                            logger.log_command(cmd_label, if stderr.is_empty() { None } else { Some(stderr) }, Some(0), Some(duration_ms));
                        } else {
                            error!("FC backup failed with status {:?} for {}", o.status, cfg.name);
            ...
            PostgresDumpFormat::Fd => {
                ...
                if let Err(e) = std::fs::create_dir_all(&dump_dir) {
                    error!("Failed to create dump directory {:?} for {}: {:?}", dump_dir, cfg.name, e);
                    return Err(e.into());
                }
                ...
                match output {
                    Ok(o) => {
                        ...
                        if o.status.success() {
                            ...
                            error!("FD backup pg_dump failed with status {:?} for {}", o.status, cfg.name);
                            logger.log_command(cmd_label, Some(stderr), Some(exit_code), Some(duration_ms));
                ...
                match std::fs::File::create(&tar_file) {
                    ...
                    Err(e) => {
                        error!("Failed to create tar.gz file {:?} for {}: {:?}", tar_file, cfg.name, e);
                        return Err(e.into());
                    }
                }
                logger.log("info", format!("Backup finished for database {}", cfg.name));
